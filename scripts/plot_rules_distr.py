...
def plot_rules_distr_ms():
    ...
    def _get_color_pl(pl):
        b, o, g, r = [plt.cm.Blues, plt.cm.Oranges, plt.cm.Greens, plt.cm.Reds]
        if pl == 'PL1':
            return 
    ...
    total_rules = crs_rules_activated['pl4']

    warn_rules = [942430, 942420, 942431, 942460, 942421, 942432]
    critical_rules = [rule for rule in total_rules if rule not in warn_rules]
    ...
    for pl, rules in crs_rules_unique.items():
        data = {'critical': [], 'warn': []}
        for rule in rules:
    ...
    # colors=[b(0.6), o(0.6), g(0.6), r(0.6)],
    ...
    for i, (p, label) in enumerate(zip(wedges_outer[0], labels_outer)):
        ...
        ax_rules_weights.annotate(label, xy=(x, y), xytext=(1.35*np.sign(x), 1.4*y), fontsize=12,
                                horizontalalignment=horizontalalignment, **kw)

    # inner slices
    type_to_color = {
        'CRITICAL': 'tomato', 'WARN': 'limegreen'
    }
    inner_values, inner_colors = [], []
    ...
    wedges, labels_list = ax_rules_weights.pie(inner_values, radius=1-size, colors=inner_colors,
        wedgeprops=dict(width=size, edgecolor='w'), labels=inner_labels, labeldistance=0.76, rotatelabels=False)

    for idx, label in enumerate(labels_list):
        label.update({"horizontalalignment": "center",  "verticalalignment": "center"})

    ax_rules_weights.legend(wedges[1:3], ['CRITICAL (5)', 'WARNING (3)'],
                            loc='upper center', bbox_to_anchor=(0.5, -0.005), fancybox=True, shadow=True, ncol=4, fontsize=12) # bbox_to_anchor=(1, 0, 0.5, 1)
