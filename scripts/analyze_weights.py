...
def analyze_weights_without_modsec(
    model_name,
    model_l1,
    model_l2,
    crs_ids,
    ...
    The weights are compared for each CRS rule.
    ...
    df_plot = pd.DataFrame(
        {
            'rules': crs_ids * 2,
            'weight': model_l1_weights.tolist() + model_l2_weights.tolist(),
            'type': [f'AdvModSec ({model_name})'] * len(crs_ids) + [f'MLModSec ({model_name})'] * len(crs_ids)
        }
    )

    so.Plot(df_plot, x = 'rules', y = 'weight', color = 'type')\
        .add(so.Bar()) \
        .scale(color=['#81b8ef', '#fe6d73']) \
        .on(axs) \
        .plot()
...
def analyze_weights_with_modsec(
    model_name,
    model_l1,
    model_l2,
    crs_ids,
    ...
    df_plot = pd.DataFrame(
        {
            ...
            'type': ['ModSec'] * len(crs_ids) + 
                    [f'AdvModSec ({model_name})'] * len(crs_ids) + 
                    [f'MLModSec ({model_name}) '] * len(crs_ids)
        }
    )
...
if __name__ == '__main__':
    ...
    # Linear SVC
    model_name = 'adv_linear_svc_pl{}_l1.joblib'.format(pl)
    ...
    analyze_weights_without_modsec(
        'SVM',
        model_l1,
        model_l2,
        crs_ids,
